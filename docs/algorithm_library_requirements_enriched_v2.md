# Algorithm Library Requirements — Version 2

## What changed in this version

This version keeps the full library, preserves the enriched fields you previously asked for, and additionally:
...
6. **22 new algorithm rows** starting at **100** and extending the library with missing factor, low-risk, fixed-income-relative-value, and event-driven strategies
7. **A second table** for established methodologies used to combine multiple algorithms into one composite system
...
<tr>
<td>100</td>
<td>Factor / Risk Premia</td>
<td><strong>Low Volatility Strategy</strong></td>
<td>No</td>
<td>Long-term investing / monthly rotation / swing portfolio</td>
<td>1</td>
<td>Prefer lower-volatility assets or equities over higher-volatility peers.</td>
<td>Universe returns, realized volatility or ATR estimates, rebalance schedule, optional sector/country constraints</td>
<td>Cross-sectional rank, portfolio-selection, or long/short signal</td>
<td>Implement this as a cross-sectional or portfolio sleeve rather than a single-candle trigger. On each rebalance date, compute the required volatility descriptors from your normalized return history—such as realized volatility, ATR-style realized range, or model-based residual volatility depending on the specific variant—winsorize or clip obvious outliers, standardize the metric across the eligible universe, and either rank names directly or transform the measure into a portfolio weight. If you are using it as a long-only strategy, prefer the lowest-risk names subject to sector, liquidity, and concentration constraints. If you are using it as a long/short factor, form top-versus-bottom buckets and beta-neutralize if needed. Preserve intermediate series, rebalance timestamps, and eligibility flags in the report payload so the dashboard can explain why a security was selected or excluded.</td>
<td><code>MSCI-MINVOL</code></td>
</tr>
<tr>
<td>101</td>
<td>Factor / Risk Premia</td>
<td><strong>Minimum Variance Strategy</strong></td>
<td>No</td>
<td>Long-term investing / portfolio construction</td>
<td>2</td>
<td>Construct a portfolio that minimizes estimated total variance subject to constraints.</td>
<td>Universe returns, covariance matrix, optimization constraints, rebalance schedule</td>
<td>Optimized portfolio weights rather than a single bar-level trigger</td>
<td>Treat this as an optimization algorithm. Build an eligible universe first, estimate a covariance matrix from rolling returns or your preferred risk model, and solve a constrained optimization problem whose objective is minimum total portfolio variance. Add practical constraints such as max weight, min weight, turnover cap, sector or country neutrality, diversification limits, and optional tracking-error caps. Because covariance estimates are noisy, the coder should include shrinkage or stabilization options and warm-up requirements. The output should be a weight vector plus diagnostics such as predicted volatility, marginal risk contribution, and the binding constraints used by the optimizer.</td>
<td><code>MSCI-MINVOL</code></td>
</tr>
<tr>
<td>102</td>
<td>Factor / Risk Premia</td>
<td><strong>Residual Volatility Strategy</strong></td>
<td>No</td>
<td>Long-term investing / monthly rotation / swing portfolio</td>
<td>2</td>
<td>Prefer securities with lower residual/idiosyncratic volatility after accounting for market or factor exposure.</td>
<td>Universe returns, factor model or market model, residual volatility estimates, rebalance schedule</td>
<td>Cross-sectional rank or filtered buy list</td>
<td>Implement this as a cross-sectional or portfolio sleeve rather than a single-candle trigger. On each rebalance date, compute the required volatility descriptors from your normalized return history—such as realized volatility, ATR-style realized range, or model-based residual volatility depending on the specific variant—winsorize or clip obvious outliers, standardize the metric across the eligible universe, and either rank names directly or transform the measure into a portfolio weight. If you are using it as a long-only strategy, prefer the lowest-risk names subject to sector, liquidity, and concentration constraints. If you are using it as a long/short factor, form top-versus-bottom buckets and beta-neutralize if needed. Preserve intermediate series, rebalance timestamps, and eligibility flags in the report payload so the dashboard can explain why a security was selected or excluded.</td>
<td><code>MSCI-FACS</code></td>
</tr>
<tr>
<td>103</td>
<td>Factor / Risk Premia</td>
<td><strong>Low Beta / Betting-Against-Beta</strong></td>
<td>No</td>
<td>Long-term investing / swing portfolio / factor sleeve</td>
<td>2</td>
<td>Go long low-beta assets and short or underweight high-beta assets.</td>
<td>Universe returns, rolling beta estimates, leverage rules, rebalance schedule</td>
<td>Cross-sectional long/short factor signal</td>
<td>Implement this as a universe-level factor sleeve. Estimate rolling beta and related risk measures on a consistent window, optionally smooth them, then combine them with any additional descriptors required by the strategy—such as quality metrics for the defensive-equity version. Build a ranked score, apply liquidity and borrow/shortability rules if you support long/short deployment, and generate either long-only tilted weights or long/short dollar-neutral exposures. Keep the raw beta, volatility, and any quality sub-scores in the report, because these strategies are best understood as transparent factor combinations rather than single threshold rules.</td>
<td><code>AQR-BAB</code></td>
</tr>
<tr>
<td>104</td>
<td>Factor / Risk Premia</td>
<td><strong>Defensive Equity Strategy</strong></td>
<td>No</td>
<td>Long-term investing / defensive allocation</td>
<td>2</td>
<td>Combine low-risk and high-quality characteristics to build a more defensive equity sleeve.</td>
<td>Universe prices, beta/volatility measures, quality descriptors, portfolio constraints</td>
<td>Ranked defensive score and portfolio allocation</td>
<td>Implement this as a universe-level factor sleeve. Estimate rolling beta and related risk measures on a consistent window, optionally smooth them, then combine them with any additional descriptors required by the strategy—such as quality metrics for the defensive-equity version. Build a ranked score, apply liquidity and borrow/shortability rules if you support long/short deployment, and generate either long-only tilted weights or long/short dollar-neutral exposures. Keep the raw beta, volatility, and any quality sub-scores in the report, because these strategies are best understood as transparent factor combinations rather than single threshold rules.</td>
<td><code>AQR-DEF</code></td>
</tr>
<tr>
<td>105</td>
<td>Factor / Risk Premia</td>
<td><strong>Size / Small-Cap Strategy</strong></td>
<td>No</td>
<td>Long-term investing / monthly or quarterly rotation</td>
<td>2</td>
<td>Tilt toward smaller-cap securities, typically with liquidity filters.</td>
<td>Market-cap data, universe definitions, liquidity filters, rebalance schedule</td>
<td>Cross-sectional rank or bucket selection</td>
<td>Implement this as a periodic cross-sectional ranking strategy. On each rebalance date, compute the required descriptor set from the latest point-in-time data available to the model, standardize or percentile-rank the descriptors across the eligible universe, and map them into one composite score for the targeted factor. Then either select the top bucket, overweight stronger names, or form long-versus-short buckets if your framework supports factor-neutral deployment. The coder should explicitly protect against look-ahead bias by using reporting-lag assumptions for fundamentals, and should include optional neutralization by sector, country, beta, or size where appropriate. The report should expose the raw descriptor values, standardized scores, final rank, and any names excluded for liquidity or data quality reasons.</td>
<td><code>MSCI-FACS</code></td>
</tr>
<tr>
<td>106</td>
<td>Factor / Risk Premia</td>
<td><strong>Mid-Cap Tilt Strategy</strong></td>
<td>No</td>
<td>Long-term investing / periodic rotation</td>
<td>2</td>
<td>Tilt toward mid-cap names as a distinct style sleeve rather than only large or small cap.</td>
<td>Market-cap data, band definitions, liquidity filters, rebalance schedule</td>
<td>Bucket-selection / rank-based signal</td>
<td>Implement this as a periodic cross-sectional ranking strategy. On each rebalance date, compute the required descriptor set from the latest point-in-time data available to the model, standardize or percentile-rank the descriptors across the eligible universe, and map them into one composite score for the targeted factor. Then either select the top bucket, overweight stronger names, or form long-versus-short buckets if your framework supports factor-neutral deployment. The coder should explicitly protect against look-ahead bias by using reporting-lag assumptions for fundamentals, and should include optional neutralization by sector, country, beta, or size where appropriate. The report should expose the raw descriptor values, standardized scores, final rank, and any names excluded for liquidity or data quality reasons.</td>
<td><code>MSCI-FACS</code></td>
</tr>
<tr>
<td>107</td>
<td>Factor / Risk Premia</td>
<td><strong>Dividend Yield Strategy</strong></td>
<td>No</td>
<td>Long-term income investing / monthly or quarterly rotation</td>
<td>1</td>
<td>Select assets or equities with higher dividend yield, often with quality and payout sanity checks.</td>
<td>Dividend yield, payout ratio, prices, optional quality filters</td>
<td>Cross-sectional rank or income-oriented buy list</td>
<td>Implement this as a periodic cross-sectional ranking strategy. On each rebalance date, compute the required descriptor set from the latest point-in-time data available to the model, standardize or percentile-rank the descriptors across the eligible universe, and map them into one composite score for the targeted factor. Then either select the top bucket, overweight stronger names, or form long-versus-short buckets if your framework supports factor-neutral deployment. The coder should explicitly protect against look-ahead bias by using reporting-lag assumptions for fundamentals, and should include optional neutralization by sector, country, beta, or size where appropriate. The report should expose the raw descriptor values, standardized scores, final rank, and any names excluded for liquidity or data quality reasons.</td>
<td><code>MSCI-FACS</code></td>
</tr>
<tr>
<td>108</td>
<td>Factor / Risk Premia</td>
<td><strong>Growth Factor Strategy</strong></td>
<td>No</td>
<td>Long-term investing / quarterly rotation</td>
<td>2</td>
<td>Prefer companies with stronger growth characteristics such as earnings or sales growth.</td>
<td>Fundamental growth metrics, prices, rebalance schedule</td>
<td>Cross-sectional rank / portfolio selection</td>
<td>Implement this as a periodic cross-sectional ranking strategy. On each rebalance date, compute the required descriptor set from the latest point-in-time data available to the model, standardize or percentile-rank the descriptors across the eligible universe, and map them into one composite score for the targeted factor. Then either select the top bucket, overweight stronger names, or form long-versus-short buckets if your framework supports factor-neutral deployment. The coder should explicitly protect against look-ahead bias by using reporting-lag assumptions for fundamentals, and should include optional neutralization by sector, country, beta, or size where appropriate. The report should expose the raw descriptor values, standardized scores, final rank, and any names excluded for liquidity or data quality reasons.</td>
<td><code>MSCI-FACS</code></td>
</tr>
<tr>
<td>109</td>
<td>Factor / Risk Premia</td>
<td><strong>Liquidity Factor Strategy</strong></td>
<td>No</td>
<td>Long-term investing / execution-aware portfolio construction</td>
<td>2</td>
<td>Use liquidity metrics either as a selection signal or as a filter for tradability and crowding control.</td>
<td>Volume, turnover, bid-ask proxies, market cap, rebalance schedule</td>
<td>Cross-sectional score or tradability filter</td>
<td>Implement this as a periodic cross-sectional ranking strategy. On each rebalance date, compute the required descriptor set from the latest point-in-time data available to the model, standardize or percentile-rank the descriptors across the eligible universe, and map them into one composite score for the targeted factor. Then either select the top bucket, overweight stronger names, or form long-versus-short buckets if your framework supports factor-neutral deployment. The coder should explicitly protect against look-ahead bias by using reporting-lag assumptions for fundamentals, and should include optional neutralization by sector, country, beta, or size where appropriate. The report should expose the raw descriptor values, standardized scores, final rank, and any names excluded for liquidity or data quality reasons.</td>
<td><code>MSCI-FACS</code></td>
</tr>
<tr>
<td>110</td>
<td>Factor / Risk Premia</td>
<td><strong>Profitability Factor Strategy</strong></td>
<td>No</td>
<td>Long-term investing / quarterly rotation</td>
<td>2</td>
<td>Prefer firms with stronger profitability metrics.</td>
<td>ROE/ROA/gross profitability/margins, prices, rebalance schedule</td>
<td>Cross-sectional rank / portfolio selection</td>
<td>Implement this as a periodic cross-sectional ranking strategy. On each rebalance date, compute the required descriptor set from the latest point-in-time data available to the model, standardize or percentile-rank the descriptors across the eligible universe, and map them into one composite score for the targeted factor. Then either select the top bucket, overweight stronger names, or form long-versus-short buckets if your framework supports factor-neutral deployment. The coder should explicitly protect against look-ahead bias by using reporting-lag assumptions for fundamentals, and should include optional neutralization by sector, country, beta, or size where appropriate. The report should expose the raw descriptor values, standardized scores, final rank, and any names excluded for liquidity or data quality reasons.</td>
<td><code>MSCI-FACS</code></td>
</tr>
...
<td>111</td>
<td>Factor / Risk Premia</td>
<td><strong>Earnings Quality Strategy</strong></td>
<td>No</td>
<td>Long-term investing / quarterly rotation</td>
<td>2</td>
<td>Prefer firms whose reported earnings appear more persistent and lower-noise.</td>
<td>Accruals, cash-flow support, earnings data, prices, rebalance schedule</td>
<td>Cross-sectional rank / quality score</td>
<td>Implement this as a periodic cross-sectional ranking strategy. On each rebalance date, compute the required descriptor set from the latest point-in-time data available to the model, standardize or percentile-rank the descriptors across the eligible universe, and map them into one composite score for the targeted factor. Then either select the top bucket, overweight stronger names, or form long-versus-short buckets if your framework supports factor-neutral deployment. The coder should explicitly protect against look-ahead bias by using reporting-lag assumptions for fundamentals, and should include optional neutralization by sector, country, beta, or size where appropriate. The report should expose the raw descriptor values, standardized scores, final rank, and any names excluded for liquidity or data quality reasons.</td>
<td><code>MSCI-FACS</code></td>
</tr>
<tr>
<td>112</td>
<td>Factor / Risk Premia</td>
<td><strong>Investment Quality Strategy</strong></td>
<td>No</td>
<td>Long-term investing / quarterly rotation</td>
<td>2</td>
<td>Prefer firms that invest capital more efficiently and avoid value-destructive expansion.</td>
<td>Asset growth, capex/investment metrics, balance-sheet data, prices</td>
<td>Cross-sectional rank / quality score</td>
<td>Implement this as a periodic cross-sectional ranking strategy. On each rebalance date, compute the required descriptor set from the latest point-in-time data available to the model, standardize or percentile-rank the descriptors across the eligible universe, and map them into one composite score for the targeted factor. Then either select the top bucket, overweight stronger names, or form long-versus-short buckets if your framework supports factor-neutral deployment. The coder should explicitly protect against look-ahead bias by using reporting-lag assumptions for fundamentals, and should include optional neutralization by sector, country, beta, or size where appropriate. The report should expose the raw descriptor values, standardized scores, final rank, and any names excluded for liquidity or data quality reasons.</td>
<td><code>MSCI-FACS</code></td>
</tr>
...
<td>113</td>
<td>Factor / Risk Premia</td>
<td><strong>Low Leverage / Balance-Sheet Strength</strong></td>
<td>No</td>
<td>Long-term investing / defensive rotation</td>
<td>2</td>
<td>Prefer companies with lower leverage and stronger balance-sheet resilience.</td>
<td>Debt ratios, interest coverage, balance-sheet metrics, prices</td>
<td>Cross-sectional rank / defensive filter</td>
<td>Implement this as a periodic cross-sectional ranking strategy. On each rebalance date, compute the required descriptor set from the latest point-in-time data available to the model, standardize or percentile-rank the descriptors across the eligible universe, and map them into one composite score for the targeted factor. Then either select the top bucket, overweight stronger names, or form long-versus-short buckets if your framework supports factor-neutral deployment. The coder should explicitly protect against look-ahead bias by using reporting-lag assumptions for fundamentals, and should include optional neutralization by sector, country, beta, or size where appropriate. The report should expose the raw descriptor values, standardized scores, final rank, and any names excluded for liquidity or data quality reasons.</td>
<td><code>MSCI-FACS</code></td>
</tr>
<tr>
<td>114</td>
<td>Factor / Risk Premia</td>
<td><strong>Earnings Stability / Low Earnings Variability</strong></td>
<td>No</td>
<td>Long-term investing / defensive quality sleeve</td>
<td>2</td>
<td>Prefer firms with more stable earnings and penalize highly erratic earnings histories.</td>
<td>Historical earnings series, variability measures, prices</td>
<td>Cross-sectional rank / stability score</td>
<td>Implement this as a periodic cross-sectional ranking strategy. On each rebalance date, compute the required descriptor set from the latest point-in-time data available to the model, standardize or percentile-rank the descriptors across the eligible universe, and map them into one composite score for the targeted factor. Then either select the top bucket, overweight stronger names, or form long-versus-short buckets if your framework supports factor-neutral deployment. The coder should explicitly protect against look-ahead bias by using reporting-lag assumptions for fundamentals, and should include optional neutralization by sector, country, beta, or size where appropriate. The report should expose the raw descriptor values, standardized scores, final rank, and any names excluded for liquidity or data quality reasons.</td>
<td><code>MSCI-FACS</code></td>
</tr>
<tr>
<td>115</td>
<td>Fixed Income Relative Value</td>
<td><strong>Fixed-Income Arbitrage</strong></td>
<td>Yes</td>
<td>Short-term to medium-term relative-value trading</td>
<td>3</td>
<td>Exploit pricing discrepancies between related debt instruments while hedging duration or other common risks.</td>
<td>Yield curves, bond prices, spreads, financing/carry assumptions, hedge ratios</td>
<td>Relative-value spread signal with paired positions</td>
<td>Implement this as a relative-value engine rather than a directional rates strategy. The core workflow is: identify closely related fixed-income instruments, estimate their fair-value relationship after adjusting for duration, convexity, carry, financing, and optional credit exposure, then compute a spread or residual mispricing series. Open paired or basket positions when the spread deviates enough from fair value and close when it converges or risk limits are hit. The coder should support hedge-ratio estimation, carry accounting, financing assumptions, stop-loss and stress logic, and explicit reporting of duration exposure, spread z-score, convergence target, and mark-to-market decomposition. This strategy is usually capital-, leverage-, and data-intensive, so the implementation should also make operational assumptions visible in the report.</td>
<td><code>INV-FIARB</code></td>
</tr>
<tr>
<td>116</td>
<td>Fixed Income Relative Value</td>
<td><strong>Swap Spread Arbitrage</strong></td>
<td>Yes</td>
<td>Short-term to medium-term relative-value trading</td>
<td>3</td>
<td>Trade deviations between swap spreads and comparable government-bond yields.</td>
<td>Swap rates, sovereign yields, maturity matching, carry and funding assumptions</td>
<td>Relative-value spread signal with hedged legs</td>
<td>Implement this as a specific fixed-income relative-value strategy. Match swap maturities to the corresponding sovereign-bond points, calculate the swap spread time series, normalize the spread with history-aware statistics, and estimate whether the deviation is large enough to justify a convergence trade after financing and transaction costs. The algorithm should model hedge ratios carefully, because duration, convexity, and funding assumptions matter. Reporting should include the live spread, rolling z-score, hedge ratio, carry estimate, and any stress-limit or funding-rule violations. This is not a pure retail indicator strategy; it behaves more like a model-based spread engine.</td>
<td><code>INV-SWAPSP</code></td>
</tr>
<tr>
<td>117</td>
<td>Event-Driven</td>
<td><strong>Post-Earnings Announcement Drift (PEAD)</strong></td>
<td>No</td>
<td>Event-driven / short- to medium-term after earnings</td>
<td>2</td>
<td>Go with the direction of the earnings surprise after the announcement because prices often continue to drift.</td>
<td>Earnings calendar, surprise measure (SUE/EAR/analyst surprise), prices, holding window</td>
<td>Event-driven buy/sell signal after announcement</td>
<td>Implement this as an event-driven strategy keyed to the earnings calendar. For each announcement, compute an earnings-surprise measure such as standardized unexpected earnings, analyst-surprise residual, or announcement-window return surprise, then open a position after the announcement in the direction implied by the surprise. The holding horizon, skip window, and ranking scheme should be configurable, because PEAD variants differ mainly in surprise definition and holding period. To keep the implementation valid, the coder must use point-in-time event timestamps, avoid look-ahead bias around after-hours announcements, and expose event metadata in the report: announcement time, surprise score, entry timestamp, exit timestamp, and abnormal-return decomposition if available.</td>
<td><code>QPEDIA-PEAD</code></td>
</tr>
<tr>
<td>118</td>
<td>Event-Driven</td>
<td><strong>Pre-Earnings Announcement Drift</strong></td>
<td>No</td>
<td>Event-driven / very short-term before earnings</td>
<td>2</td>
<td>Exploit the tendency for some stocks to move predictably before the next earnings date based on prior earnings behavior.</td>
<td>Earnings calendar, prior announcement returns or earnings-behavior features, prices</td>
<td>Pre-event long/short or buy/watchlist signal</td>
<td>Implement this as a pre-event anomaly strategy. The model should maintain an earnings calendar, derive a pre-announcement signal from prior earnings behavior or announcement-related return patterns, and open a position in the defined window before the next scheduled earnings release. Because the whole strategy depends on schedule accuracy, the coder should treat event timestamps, missed reschedulings, and holiday effects carefully. Include configurable entry and exit windows, liquidity filters, and optional combination with analyst-revision or momentum filters. The report should show the event date, pre-event window, signal source, and any event-level exclusions.</td>
<td><code>QPEDIA-PREAD</code></td>
</tr>
<tr>
<td>119</td>
<td>Event-Driven</td>
<td><strong>Earnings Announcement Premium</strong></td>
<td>No</td>
<td>Event-driven / around scheduled earnings windows</td>
<td>2</td>
<td>Exploit the tendency for some stocks to earn abnormal returns concentrated around announcement dates.</td>
<td>Earnings calendar, historical announcement activity/volume, prices</td>
<td>Scheduled event-window allocation signal</td>
<td>Implement this as a calendar-based event allocation strategy. The algorithm should identify stocks whose returns historically concentrate around announcement windows, score them using announcement-related activity or premium proxies, and enter positions only during the chosen event window. Compared with PEAD, the focus here is not just surprise direction but the tendency for the event window itself to carry abnormal returns. The coder should support monthly batch processing of all upcoming announcements, configurable entry/exit offsets, and clear reporting of the event-window contribution to total PnL.</td>
<td><code>QPEDIA-EAP</code></td>
</tr>
<tr>
<td>120</td>
<td>Event-Driven</td>
<td><strong>Index Rebalancing Effect Strategy</strong></td>
<td>No</td>
<td>Event-driven / short-term around index rebalances</td>
<td>2</td>
<td>Trade predictable price pressure around known index reconstitution and rebalance events.</td>
<td>Index rebalance calendars, constituent change lists, prices, volume, market-cap data</td>
<td>Event-driven buy/sell around rebalance dates</td>
<td>Implement this as a rules-based event strategy around known reconstitution and rebalance calendars. The engine should ingest the public methodology and announced constituent changes, classify additions, deletions, and expected weight changes, and then apply a configurable trading policy before, on, or after the effective rebalance date. Because price pressure and liquidity matter, the coder should track announcement dates, effective dates, average daily volume, and rebalance size relative to float. The report should explain every trade through the rebalance event metadata and should distinguish between the announcement effect and the effective-date flow effect.</td>
<td><code>QPEDIA-REB</code></td>
</tr>
<tr>
<td>121</td>
<td>Event-Driven</td>
<td><strong>ETF Rebalancing Anticipation / Front-Run Strategy</strong></td>
<td>Yes</td>
<td>Event-driven / short-term around ETF rebalance dates</td>
<td>2</td>
<td>Anticipate flows caused by transparent ETF or index rebalances and position before the forced trading window.</td>
<td>ETF/index methodology, announced changes, expected rebalance date, prices, liquidity data</td>
<td>Event-driven buy/sell around public rebalance announcements</td>
<td>Implement this as a more tactical event-driven flow strategy. The engine should parse transparent ETF or index methodologies, estimate which constituents must be added, reduced, or removed, and position before the forced trading window when the schedule is publicly inferable. Because this logic is sensitive to liquidity and crowding, the coder should include filters on volume, spread, announcement lead time, and expected rebalance notional. The output should include both the event classification and the estimated flow pressure so the dashboard can show not only the final trade signal but also why the model believes front-running pressure exists.</td>
<td><code>QPEDIA-ETFREB</code></td>
</tr>
</tbody>
</table>
...
These are **combination / meta-strategy methods**, not standalone alpha ideas. They are useful when you want to merge several child algorithms into one more robust system.

<table>
<thead>
<tr>
<th>#</th><th>Method family</th><th>Methodology</th><th>Advanced / Institutional only?</th><th>Best use / horizon</th><th>Home suitability (1-3)</th><th>Core idea</th><th>Typical inputs</th><th>Combination output style</th><th>Extended implementation details</th><th>Initial reference</th>
</tr>
</thead>
<tbody>
<tr>
<td>1</td>
<td>Rule-Based Combination</td>
<td><strong>Hard Boolean Gating (AND / OR / Majority)</strong></td>
<td>No</td>
<td>All horizons if child signals are aligned</td>
<td>1</td>
<td>Combine child strategy outputs with explicit logical rules such as AND, OR, or majority vote.</td>
<td>Child signals, timestamps, optional precedence rules</td>
<td>Single composite discrete signal</td>
<td>Implement this as a generic combiner over normalized child outputs. Align all child signals on the same timestamps, map `buy/sell/neutral` to a compact internal representation, and evaluate a configurable truth table such as AND, OR, k-of-n majority, or veto logic. The coder should define clear conflict handling, stale-signal handling, and precedence rules so the output is deterministic. This is the simplest reliable combination method and a good baseline for testing more advanced ensembles.</td>
<td><code>CFA-ENS</code></td>
</tr>
<tr>
<td>2</td>
<td>Rule-Based Combination</td>
<td><strong>Weighted Linear Score Blend</strong></td>
<td>No</td>
<td>All horizons; especially good for swing / daily systems</td>
<td>1</td>
<td>Normalize each child score and combine them with fixed or dynamic weights into one master score.</td>
<td>Child scores, weights, normalization method, thresholds</td>
<td>Composite numeric score then buy/sell/neutral</td>
<td>Implement this by first normalizing every child model output to a common range, such as z-scores, bounded scores, or calibrated probabilities, and then computing a weighted sum. Convert the composite score to `buy/sell/neutral` with configurable thresholds and dead zones. Support both static weights and dynamic weights from a simple regime map or recent validation performance. The report should show each child contribution, the final blended score, and the exact threshold logic used to trigger a trade.</td>
<td><code>CFA-ENS</code></td>
</tr>
<tr>
<td>3</td>
<td>Rule-Based Combination</td>
<td><strong>Rank Aggregation</strong></td>
<td>No</td>
<td>Cross-sectional strategies / portfolio rotation</td>
<td>1</td>
<td>Convert child models into ranks and aggregate the ranks to reduce scale mismatch across models.</td>
<td>Per-asset ranks from child strategies, aggregation rule</td>
<td>Final rank or selection list</td>
<td>Implement this when child strategies live on different scales or produce only relative preferences. Convert each child output into a cross-sectional rank, aggregate the ranks with average-rank, median-rank, or weighted-rank logic, and select the final portfolio or watchlist from the aggregate ordering. This method is robust when one model outputs probabilities and another outputs factor scores, because ranking removes scale mismatches. Expose per-child ranks and the final aggregate rank in the report.</td>
<td><code>CFA-ENS</code></td>
</tr>
<tr>
<td>4</td>
<td>Optimization-Based</td>
<td><strong>Constrained Multi-Factor Optimization</strong></td>
<td>Yes</td>
<td>Long-term and medium-term portfolio construction</td>
<td>2</td>
<td>Optimize combined exposure to several factors or signals while controlling active risk, beta, turnover, and concentration.</td>
<td>Signal exposures, risk model, covariance matrix, optimization constraints</td>
<td>Optimized portfolio weights</td>
<td>Implement this as a bottom-up optimizer. Start from child signals or factor exposures, define the objective—such as maximizing combined exposure per unit of active risk—and then solve for weights under explicit constraints: turnover, active risk, beta, sector or country neutrality, concentration limits, liquidity, and maximum name weights. This is the most defensible way to combine several medium-term styles into one portfolio when you have a covariance model. The report should include the chosen weights, the objective value, risk statistics, turnover estimate, and which constraints were binding at the solution.</td>
<td><code>MSCI-COREMF</code></td>
</tr>
<tr>
<td>5</td>
<td>Risk Overlay</td>
<td><strong>Risk Budgeting / Risk Parity</strong></td>
<td>No</td>
<td>Portfolio-level combination across horizons</td>
<td>1</td>
<td>Allocate capital so no single strategy sleeve dominates total portfolio risk.</td>
<td>Sleeve returns, volatility/covariance estimates, risk targets</td>
<td>Portfolio weights or sleeve weights</td>
<td>Implement this at the sleeve level rather than the single-security level. Estimate each child strategy’s volatility and covariance, then solve for weights so each sleeve contributes a balanced share of total portfolio risk. Support optional leverage caps, floor weights, and rebalance bands to limit turnover. The key reporting outputs are current sleeve weights, target risk contributions, realized risk contributions, and how far the live portfolio has drifted from its target risk budget.</td>
<td><code>AQR-RP</code></td>
</tr>
<tr>
<td>6</td>
<td>Risk Overlay</td>
<td><strong>Volatility Targeting Overlay</strong></td>
<td>No</td>
<td>All horizons; especially portfolio and sleeve management</td>
<td>1</td>
<td>Scale combined positions up or down to target a stable realized volatility level.</td>
<td>Strategy returns, volatility estimate, target-vol setting</td>
<td>Risk-scaled position multiplier</td>
<td>Implement this as an overlay that rescales the gross exposure of a strategy or combination of strategies. Compute realized or forecast volatility on a rolling window, compare it with the desired target, and multiply positions by a scaling factor subject to caps, floors, and smoothing. This method is simple but operationally important because it stabilizes risk across time. The report should show forecast vol, target vol, applied multiplier, and any guardrails that clipped the scaling factor.</td>
<td><code>AQR-STYLE</code></td>
</tr>
<tr>
<td>7</td>
<td>Adaptive / State-Based</td>
<td><strong>Regime Switching / HMM Gating</strong></td>
<td>No</td>
<td>All horizons; especially when strategies behave differently in trend/range/high-vol regimes</td>
<td>2</td>
<td>Detect market regime first and activate only the strategies that historically work best in that regime.</td>
<td>Returns, volatility, macro or market-state features, regime model</td>
<td>Regime label plus gated strategy weights/signals</td>
<td>Implement this as a two-stage controller. Stage one estimates regime—trend, range, crash, calm, high-volatility, risk-on, risk-off, or any custom taxonomy—using returns, volatility, breadth, or macro features. Stage two maps each detected regime to a set of active child strategies and their weights. Hidden Markov models, state-space models, or simpler classifiers can all work as the regime engine. The report should make the regime path and the strategy-activation map explicit so users can diagnose why a strategy was enabled or disabled.</td>
<td><code>CFA-RL</code></td>
</tr>
<tr>
<td>8</td>
<td>Machine Learning Ensemble</td>
<td><strong>Bagging Ensemble</strong></td>
<td>No</td>
<td>Daily to medium-term predictive systems</td>
<td>2</td>
<td>Train multiple versions of a model on resampled data and average their outputs to reduce variance and overfitting.</td>
<td>Training data, bootstrap samples, child learner, validation setup</td>
<td>Averaged forecast or vote</td>
<td>Implement this on top of any probabilistic or score-based child model. Create bootstrap samples or resampled training folds, train one learner on each sample, and average the resulting predictions. Bagging is most useful when the base learner is noisy or unstable, because averaging reduces variance. The coder should keep out-of-sample evaluation separated cleanly from training and expose ensemble spread or model disagreement as an additional confidence metric.</td>
<td><code>CFA-ENS</code></td>
</tr>
<tr>
<td>9</td>
<td>Machine Learning Ensemble</td>
<td><strong>Boosting Ensemble</strong></td>
<td>No</td>
<td>Daily to medium-term predictive systems</td>
<td>2</td>
<td>Train models sequentially so each new learner focuses on errors made by prior learners.</td>
<td>Training data, weak learner, loss function, tuning parameters</td>
<td>Boosted forecast score</td>
<td>Implement this as a sequential ensemble in which each learner focuses on the residual error or loss left by earlier learners. Gradient boosting trees are the most practical default if you are combining tabular financial features. Because boosting can overfit, the implementation should include early stopping, learning-rate controls, depth limits, and walk-forward validation. The output should include feature importance or SHAP-style diagnostics if available so the dashboard remains explainable.</td>
<td><code>CFA-ENS</code></td>
</tr>
<tr>
<td>10</td>
<td>Machine Learning Ensemble</td>
<td><strong>Stacking / Meta-Learning</strong></td>
<td>No</td>
<td>Daily to medium-term predictive systems</td>
<td>2</td>
<td>Feed several child model outputs into a second-stage model that learns when each model is trustworthy.</td>
<td>Out-of-fold child predictions, meta-features, meta-model</td>
<td>Meta-model forecast or final signal</td>
<td>Implement this as a meta-model over child strategy outputs. First generate out-of-fold predictions from the child models so the meta-model sees realistic rather than in-sample inputs. Then train a second-stage model—linear, tree-based, or probabilistic—to learn when each child strategy is more trustworthy. This is usually the strongest general-purpose combination method when you have enough clean training data. The report should show child outputs, meta-features, meta-model prediction, and the contribution of each child model.</td>
<td><code>CFA-ENS</code></td>
</tr>
<tr>
<td>11</td>
<td>Reinforcement Learning</td>
<td><strong>RL Allocation Controller</strong></td>
<td>Yes</td>
<td>Portfolio allocation and execution over repeated decisions</td>
<td>3</td>
<td>Use reinforcement learning to allocate among strategies or adjust exposures based on state, action, and reward feedback.</td>
<td>State features, reward function, action space, simulator or historical environment</td>
<td>Dynamic weights / actions over time</td>
<td>Implement this only if you are comfortable with simulation quality and governance. Define the state as current positions, recent returns, volatility, regime indicators, and possibly macro features; define the action as strategy weights or exposure changes; and define the reward as a risk-adjusted PnL function net of costs. Train in a controlled environment with realistic friction and position constraints, then evaluate with strict walk-forward testing. The report should capture the observed state, chosen action, reward, and any safety override that clipped the action.</td>
<td><code>CFA-RL</code></td>
</tr>
<tr>
<td>12</td>
<td>Reinforcement Learning</td>
<td><strong>Hierarchical Controller / Meta-Policy</strong></td>
<td>Yes</td>
<td>Complex multi-layer systems and multi-horizon control</td>
<td>3</td>
<td>Use a top-level controller to choose regime, gross exposure, or strategy family and a lower layer to pick concrete allocations or actions.</td>
<td>State features, hierarchical action design, reward functions, child-policy outputs</td>
<td>Layered allocation or action plan</td>
<td>Implement this as a layered policy architecture. The top layer chooses a high-level mode—such as regime, gross-risk budget, or strategy family—while the lower layer translates that decision into concrete child-strategy weights or orders. This approach is valuable when you want clear separation between strategic and tactical decisions. Logging should be explicit: what the parent policy decided, what the child policies did, and how the final action was formed after constraints and risk guards were applied.</td>
<td><code>CFA-RL</code></td>
</tr>
</tbody>
</table>
...
6. **Point-in-time discipline for event/factor models:** for earnings, fundamentals, and rebalance-event strategies, explicitly model the timestamp when data became knowable to avoid leakage.
7. **Universe-aware implementation:** the new factor-style rows are mostly cross-sectional portfolio algorithms, so they should be implemented as universe-level selectors/weight builders rather than single-candle triggers.
...
- **`MSCI-FACS`** — MSCI — MSCI FaCS Methodology  
  URL: `https://www.msci.com/downloads/web/msci-com/research-and-insights/blog-post/factors-and-esg-the-truth-behind-three-myths/factors-and-esg-the-truth-behind-three-myths-MSCI-FaCS-Methodology.pdf`  
  Why used: Used for the eight factor groups and the 16 style factors such as size, momentum, beta, residual volatility, profitability, earnings quality, investment quality, leverage, yield, growth and liquidity.

- **`MSCI-MINVOL`** — MSCI — Minimum Volatility Indexes Methodology  
  URL: `https://www.msci.com/eqb/methodology/meth_docs/MSCI_Minimum_Volatility_Methodology_Sep2017.pdf`  
  Why used: Used for low-volatility and minimum-variance style strategies and optimization-based low-risk portfolios.

- **`MSCI-COREMF`** — MSCI — Core Multiple-Factor Indexes / Efficient Multi-Factor Indexing  
  URL: `https://www.msci.com/downloads/web/msci-com/research-and-insights/paper/efficient-multi-factor-indexing-and-concentrated-markets-introducing-the-msci-core-multiple-factor-indexes/Core%20Multi%20Factor%20Index_Research%20Paper.pdf`  
  Why used: Used for optimization-based combination methods that maximize combined factor exposure while controlling active risk and related constraints.

- **`AQR-STYLE`** — AQR — Understanding Style Premia  
  URL: `https://images.aqr.com/-/media/AQR/Documents/Insights/Journal-Article/Understanding-Style-Premia.pdf`  
  Why used: Used for diversified style premia, carry, defensive style, diversification by risk, and volatility targeting ideas.

- **`AQR-BAB`** — AQR — Low-Risk Investing without Industry Bets / Betting Against Beta material  
  URL: `https://www.aqr.com/-/media/AQR/Documents/Insights/Journal-Article/FAJ-LowRisk-Investing-Without-Industry-Bets.pdf`  
  Why used: Used for low-beta / betting-against-beta style implementation ideas.

- **`AQR-DEF`** — AQR — Understanding Defensive Equity  
  URL: `https://www.aqr.com/-/media/AQR/Documents/Insights/White-Papers/Understanding-Defensive-Equity.pdf`  
  Why used: Used for defensive equity concepts that combine low-risk and higher-quality exposures.

- **`AQR-RP`** — AQR — Understanding Risk Parity  
  URL: `https://www.aqr.com/-/media/AQR/Documents/Insights/White-Papers/Understanding-Risk-Parity.pdf`  
  Why used: Used for risk-budgeting and risk-parity style combination overlays.

- **`CFA-ENS`** — CFA Institute Research Foundation — Ensemble Learning in Investment: An Overview  
  URL: `https://rpc.cfainstitute.org/sites/default/files/docs/support/rf_aiinassetmanagement_practitioner-briefs_04_ensemblelearning_online.pdf`  
  Why used: Used for bagging, boosting, stacking, and broader model-combination methodology guidance.

- **`CFA-RL`** — CFA Institute Research Foundation — Reinforcement Learning and Inverse Reinforcement Learning  
  URL: `https://rpc.cfainstitute.org/sites/default/files/docs/research-reports/rf_aiinassetmanagement_06_reinforcementlearning_online.pdf`  
  Why used: Used for regime-aware state design, RL allocation controllers, and hierarchical control ideas.

- **`INV-FIARB`** — Investopedia — Fixed-Income Arbitrage Explained  
  URL: `https://www.investopedia.com/terms/f/fixedincomearbitrage.asp`  
  Why used: Used as a practitioner overview for fixed-income arbitrage and its implementation/risk profile.

- **`INV-SWAPSP`** — Investopedia — Swap Spread  
  URL: `https://www.investopedia.com/terms/s/swapspread.asp`  
  Why used: Used as a practitioner overview for swap spreads and swap-spread-based relative-value trading.

- **`QPEDIA-PEAD`** — Quantpedia — Post-Earnings Announcement Effect  
  URL: `https://quantpedia.com/strategies/post-earnings-announcement-effect`  
  Why used: Used for PEAD as a documented event-driven anomaly.

- **`QPEDIA-PREAD`** — Quantpedia — Trading Earnings / Pre-Earnings Announcement Drift tag page  
  URL: `https://quantpedia.com/strategy-tags/trading-earnings/`  
  Why used: Used as the starting source for pre-earnings announcement drift ideas.

- **`QPEDIA-EAP`** — Quantpedia — Earnings Announcement Premium  
  URL: `https://quantpedia.com/strategies/earnings-announcement-premium`  
  Why used: Used for the earnings announcement premium anomaly.

- **`QPEDIA-REB`** — Quantpedia — Should We Rebalance Index Changes Immediately?  
  URL: `https://quantpedia.com/should-we-rebalance-index-changes-immediately/`  
  Why used: Used for index rebalancing effect and reconstitution-related trading ideas.

- **`QPEDIA-ETFREB`** — Quantpedia — How to Utilize Anticipated ETF Rebalances / ETF Re-balancing and Hedge Fund Front-Running Trades  
  URL: `https://quantpedia.com/how-to-utilize-anticipated-etf-rebalances/`  
  Why used: Used for transparent ETF/index-rebalance anticipation ideas.

Total catalog size: **121 algorithms / strategy templates**.  
Combination methodologies table size: **12 methods**.