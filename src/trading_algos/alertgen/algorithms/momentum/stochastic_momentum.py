...
class StochasticMomentumAlertAlgorithm(BaseMomentumAlertAlgorithm):
    def __init__(
        ...
        *,
        ...
        super().__init__(
            f"stochastic_momentum_k={k_window}_d={d_window}",
            symbol=symbol,
    ...
    def _calculate_state(self) -> MomentumSignalState:
        ...
        bullish = k_value >= self.bullish_threshold and k_value >= d_value
        bearish = k_value <= self.bearish_threshold and k_value <= d_value
        return MomentumSignalState(
            regime=TREND.UP if bullish else TREND.DOWN if bearish else TREND.UNKNOWN,
            score=clamp_unit((k_value - 50.0) / 50.0),
            bullish=bullish,
            bearish=bearish,
            ...
            reason_code=(
                "stochastic_bullish"
                if bullish
                else "stochastic_bearish"
                if bearish
                else "stochastic_inside_threshold"
            ),
        )
