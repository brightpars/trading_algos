...
class PostEarningsAnnouncementDriftAlertAlgorithm(EventDrivenAlertAlgorithm):
    catalog_ref = "algorithm:117"


def build_post_earnings_announcement_drift_algorithm(
    *,
    algorithm_key: str,
    symbol: str,
    alg_param: dict[str, Any],
) -> PostEarningsAnnouncementDriftAlertAlgorithm:
    ...
    rows = evaluate_event_rows(
        ...
        event_value_function=lambda _row, event_metadata: (
            _extract_positive_post_event_value(
                event_metadata,
    ...
    return PostEarningsAnnouncementDriftAlertAlgorithm(
        algorithm_key=algorithm_key,
        symbol=symbol,
        alg_name="post_earnings_announcement_drift",
        family="event_driven",
...
def _extract_event_value(
    ...
    *,
    ...
    return _extract_positive_post_event_value(
        event_metadata,
        event_value_field=event_value_field,
        minimum_score_threshold=minimum_score_threshold,
    )
...
def _extract_positive_post_event_value(
    event_metadata: dict[str, Any],
    *,
    event_value_field: str,
    minimum_score_threshold: float,
