...
class RiskOnRiskOffRegimeAlertAlgorithm(CrossAssetRankingAlertAlgorithm):
    catalog_ref = "algorithm:82"


def build_risk_on_risk_off_regime_algorithm(
    *,
    algorithm_key: str,
    symbol: str,
    alg_param: dict[str, Any],
) -> RiskOnRiskOffRegimeAlertAlgorithm:
    ...
    rows = evaluate_cross_asset_ranking(
        ...
        score_label="regime_score",
    )
    return RiskOnRiskOffRegimeAlertAlgorithm(
        algorithm_key=algorithm_key,
        symbol=symbol,
        alg_name="risk_on_risk_off_regime",
