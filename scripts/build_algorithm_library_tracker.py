...
def main():
    ...
    blockers = yaml.safe_load(BLOCKERS_PATH.read_text(encoding="utf-8"))
    ...
    ready_batches = Counter(r["batch"] for r in rows if r["delivery_status"] == "ready_to_implement")
    batch_rows = [{"batch": b, "ready_rows": n} for b, n in ready_batches.most_common()]

    fixture_rows = [
        {"metric": "rows with fixture ids", "count": sum(1 for r in rows if r.get("fixture_ids"))},
        {"metric": "rows without fixture ids", "count": sum(1 for r in rows if not r.get("fixture_ids"))},
...
- Requirements document version: `{manifest['source_catalog']['requirements_doc_version']}`
...
- Total combination-method rows: **{manifest['expected_totals']['combination_methods']}**
...
{md_table(['metric', 'count'], [
    ...
    {'metric': 'combination-method rows', 'count': kind_counts['combination_method']},
    {'metric': 'ready_to_implement', 'count': status_counts['ready_to_implement']},
    {'metric': 'blocked_framework', 'count': status_counts['blocked_framework']},
...
{md_table(['family', 'kind_mix', 'total', 'ready', 'blocked', 'in_progress', 'complete'], family_rows)}

## Framework blockers summary

{md_table(['blocker_key', 'status', 'affected_rows', 'affected_families'], blocker_rows)}
